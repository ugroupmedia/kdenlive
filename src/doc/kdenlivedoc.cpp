...
void KdenliveDoc::moveTwigCodeToXml(QDomDocument doc)
{
    ...
    for (auto id : m_producerTwigCode.keys()){
        ...
        for (int j = 0; j < producerList.count(); ++j) {
            if (producerList.at(j).toElement().hasAttribute(QStringLiteral("id")) && producerList.at(j).toElement().attribute(QStringLiteral("id")) == id){
                QDomNodeList producerPropertiesList = producerList.at(j).toElement().elementsByTagName(QStringLiteral("property"));
